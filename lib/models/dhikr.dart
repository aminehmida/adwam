...
class Dhikr {
  ...
  /// The dhikr as it is said, for voice matching: [arabic] minus what is
  /// written but not pronounced (counting parentheticals, ayah roundels,
  /// ornate brackets), with ﷺ expanded. Generated by tool/build_content.py;
  /// null for user-created duas, whose [arabic] is already plain spoken
  /// text — read [spokenText], which falls back for them.
  final String? reciteText;

  /// One spoken phrase per entry in [segments]: the post-prayer and sleep
  /// tasbih change words at each run boundary, which a count alone cannot
  /// convey. Empty when the count is a plain single-phrase run.
  final List<String> reciteSegments;

  /// False when the card's Arabic is not a phrase to say at all — a narration
  /// describing an action (sl-99), or a surah shown by name and read from the
  /// mushaf (sl-110a/b). Such cards stay tap-driven.
  final bool isRecitable;
  ...
  const Dhikr({
    ...
    this.reciteText,
    this.reciteSegments = const [],
    this.isRecitable = true,
  ...
  /// What voice matching compares a recognised utterance against.
  String get spokenText => reciteText ?? arabic;
  ...
  factory Dhikr.fromJson(Map<String, dynamic> json) => Dhikr(
        ...
        reciteText: json['recite_text'] as String?,
        reciteSegments:
            (json['recite_segments'] as List?)?.cast<String>() ?? const [],
        isRecitable: json['recitable'] as bool? ?? true,
